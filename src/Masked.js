...
import { lettersState } from "./Board"
...
export default function Masked() {
    ...
    useEffect(() => {
        ...
        let phraseArray = playingWord.split('')
    ...
    const [playingWord, setplayingWord] = useState(randomWord)
    const [winningLettersArray, setwinningLettersArray] = useState([])
    const [lettersArray, setlettersArray] = useRecoilState(lettersState);
    ...
    const playing = (playingWord,lettersArray) =>{

        let phraseArray = playingWord.split('')
        let lastClickedLetter = lettersArray[lettersArray.length - 1]

    
         for(let i=0; i<phraseArray.length; i++) {
            // Si c'est le cas : 
            if (phraseArray[i]===lastClickedLetter) {
                setwinningLettersArray(oldArray => [...oldArray, lastClickedLetter]);
                
            }
         } 
    } 

    console.log(winningLettersArray);
    ...
    return (
      <>
      { showButton ? <Button resetAction = {reset}></Button> : null}
      <div className='content'>
            <h1>Le Jeu du Pendu</h1>
            <div className='game'>{computeDisplay(playingWord,lettersArray)}</div>
            <Board className='board'></Board>
            <div>{playing(playingWord,lettersArray)}</div> 
            { showButton ? <Button resetAction = {reset}></Button> : null}   
        </div> 
      </>
    )
