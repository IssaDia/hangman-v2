...
import {
  ...
  import { usedLettersArray } from "./Board"
...
export default function Masked() {
    ...
    const letterFromBoard = useRecoilValue(usedLettersArray)
    ...
    console.log(letterFromBoard);

    let computeDisplay = (phrase, usedLetters)  => {
    ...
    let getClickedLetterFromBoard = (clickedLetterFromBoard) =>{
    ...
    let wordToFound = randomWord.split("").map(function(letter) {
        return "_ ";
    })
    return (
      <>
      <h1>Le Jeu du Pendu</h1>
      {()=>computeDisplay(wordToFound, letterFromBoard)}
      <Board></Board>
      </>
    )
}
